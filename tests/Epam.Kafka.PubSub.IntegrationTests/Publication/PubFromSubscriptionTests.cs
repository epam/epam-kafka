...
public class PubFromSubscriptionTests : TestWithServices, IClassFixture<MockCluster>
{
    private readonly MockCluster _mockCluster;

    public PubFromSubscriptionTests(ITestOutputHelper output, MockCluster mockCluster) : base(output)
    {
    ...
    public async Task OnePartitionTwoBatches()
    {
        ...
        var pubsub = kafkaBuilder
            .AddPublicationFromSubscription<string, TestEntityKafka, string, TestEntityKafka, TestConversionHandler>(observer.Name, ServiceLifetime.Scoped);
        ...
        pubsub.Item2
            ...
            .WithOptions(options =>
            {
                options.DefaultTopic = $"{observer.Name}.pub";
            });
